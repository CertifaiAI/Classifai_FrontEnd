...
export class LanguageService {
  ...
  setLanguageState(language: string): void {
    const changedLangString =
      language.length > 2 ? language.slice(-2) : language;
  ...
  private setTranslation = (
    ...
    this._translate.use(
      browserLang.match(finalizedLang) ? browserLang : `${compName}-${lang}`
    );
  };
}
