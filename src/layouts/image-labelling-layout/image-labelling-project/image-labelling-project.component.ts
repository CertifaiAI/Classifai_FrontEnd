...
export class ImageLabellingProjectComponent implements OnInit, OnChanges {
  ...
  checkStateEqual = (currObj: object, prevObj: object): boolean =>
    !isEqual(currObj, prevObj);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes._thumbnailList) {
      const {
        currentValue,
      }: { currentValue: Props<IThumbnailMetadata[]> } = changes._thumbnailList;
      // console.log(currentValue);
      this._thumbnailList = Object.assign([], this._thumbnailList, [
        ...currentValue,
      ]);
    }
    ...
    if (
      changes._tabStatus &&
      this.checkStateEqual(
        changes._tabStatus.currentValue,
        changes._tabStatus.previousValue
      )
    ) {
      const {
        currentValue,
      }: { currentValue: TabsProps[] } = changes._tabStatus;
