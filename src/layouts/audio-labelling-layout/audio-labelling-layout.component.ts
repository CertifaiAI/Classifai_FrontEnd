...
import { label, RegionProps } from '../../shared/types/audio-labelling/audio-labelling.model';
...
export class AudioLabellingLayoutComponent implements OnInit, OnDestroy {
    ...
    projectName: string = '';
    fileSize: string | undefined = '1.2 MB';
    ...
    ngOnInit() {
        ...
        this.audioFileName = this.audioLabellingLayoutService.getRouteState(history).fileName;
        this.filePath = this.audioLabellingLayoutService.getRouteState(history).filePath;
        if (this.audioFileName !== undefined && this.audioFileName !== '') {
            this.fileType = this.audioFileName.split('.').pop();
        }
        this.initProject(this.projectName);
        this.languageService.initializeLanguage(`data-set-page`, this.langsArr);
    }
    ...
    fetchPeaksFromBackend() {
        this.audioLabellingLayoutService.getWaveFormPeaks(this.projectName).subscribe(
            ...
            () => {
                console.error('Fail to retrieve peaks from database');
            },
            () => {
    ...
    generateWaveform(): void {
        this.wave = WaveSurfer.create({
            container: '#waveForm',
            waveColor: '#5F9EA0', // #5F9EA0
            progressColor: '#5F9EA0', // #00BFFF
            barWidth: 2,
            barHeight: 1,
            barRadius: 3,
            barGap: 2,
        ...
        this.wave.on('ready', () => {
            ...
            const waveCanvas = waveDivs.item(0) as HTMLElement;
            const progressWaveDiv = waveDivs.item(1) as HTMLElement;
            const miniMapWaveDiv = waveDivs.item(2) as HTMLElement;
            ...
            // const clone = miniMapProgressDiv.cloneNode(true) as HTMLElement;

            playHead.id = 'playHead';
            miniMapPlayHead.id = 'miniMapPlayHead';
            timeIndicator.id = 'timeIndicator';
            // clone.id = 'cloneWave';
            // clone.style.zIndex = String(5);
            // miniMapWaveDiv.appendChild(clone);
    ...
    updateMiniMapRegionIndicator() {
        ...
        const totalLength = document.querySelectorAll('canvas').item(1).clientWidth;
        const cloneWave = document.getElementById('cloneWave');
        ...
        // if (cloneWave) {
        //     cloneWave.style.width = miniMapProgressWidth + 'px';
        // }
    ...
    onDragRegionIndicator(event: DragEvent) {
        ...
        if (regionIndicator) {
            ...
            console.log('left: ', regionRectProp.left);
            console.log('client width: ', regionIndicator.clientWidth);
            console.log('sum: ', regionRectProp.left + regionIndicator.clientWidth);
    ...
    ngOnDestroy() {
        this.wave?.destroy();
        this.resetProjectStatus(this.projectName);
    }
}
