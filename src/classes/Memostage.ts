...
export class BndStageMemo {
  ...
  public SaveStages(stages: stages) {
    try {
      ...
      console.log(
        'SaveStages(stages) in BndStageMemo ----> ',
        err.name + ': ',
        err.message
      );
    }
  ...
  public replaceStages(stages: stages) {
    try {
      if (stages && this.stages_.length > 0) {
        this.stages_[this.stages_.length - 1] = this.utils.DeepCloneObject(
          stages
        );
      }
    } catch (err) {
      console.log(
        'replaceStages(stages:stages) in BndStageMemo ----> ',
        err.name + ': ',
        err.message
      );
    }
  ...
  public ClearlastStages() {
    try {
      ...
      console.log(
        'ClearlastStages() in BndStageMemo ----> ',
        err.name + ': ',
        err.message
      );
    }
  ...
  public ClearReduncentStages() {
    try {
      if (
        this.stages_[this.stages_.length - 2] !== null &&
        this.stages_[this.stages_.length - 2] !== undefined
      ) {
        let isStage2change: boolean = this.CheckIsChanges(
          this.stages_[this.stages_.length - 2].stage.bndbox
        );
        if (!isStage2change) {
      ...
      console.log(
        'ClearReduncentStages() in BndStageMemo ----> ',
        err.name + ': ',
        err.message
      );
    }
  ...
  public getMaxStageSize() {
    try {
      ...
      console.log(
        'getMaxStageSize() in BndStageMemo ----> ',
        err.name + ': ',
        err.message
      );
      return null;
  ...
  public CheckIsChanges(BndBoxList: Boundingbox[]): boolean {
    try {
      ...
      let lastMetBoxes: Boundingbox[] = this.stages_[this.stages_.length - 1]
        .stage.bndbox;
      ...
      console.log(
        'CheckIsChanges(bndbox:Boundingbox) in BndStageMemo ----> ',
        err.name + ': ',
        err.message
      );
      return false;
  ...
  public isMethodChange(comparemethod: string): boolean {
    try {
      ...
      console.log(
        'isMethodChange() in BndStageMemo ----> ',
        err.name + ': ',
        err.message
      );
      return false;
  ...
  private isBndBoxChange(
    thisbox: Boundingbox[],
    comparebox: Boundingbox[]
  ): boolean {
    try {
  ...
  public setMaxStageSize(newStageSize: number) {
    try {
      ...
      console.log(
        'setMaxStageSize() in BndStageMemo ----> ',
        err.name + ': ',
        err.message
      );
    }
