...
export class HttpRequest {
    ...
    try {
      ...
      http.onload = function () {
        if (http.readyState == 4 && http.status == 200) {
          ...
          let parurl = self.referrer.substring(
            0,
            self.referrer.lastIndexOf(domain)
          );
  ...
  public RecursPutRequest(
    ...
    try {
      ...
      let ThisObject: Metadata = JSON.parse(
        JSON.stringify(arr_content[iter].data)
      );
