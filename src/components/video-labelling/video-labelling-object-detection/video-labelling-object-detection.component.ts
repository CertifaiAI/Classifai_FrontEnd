...
import { Direction, UndoState } from '../../../shared/types/labelling-type/image-labelling.model';
...
export class VideoLabellingObjectDetectionComponent implements OnInit, OnChanges, OnDestroy {
    ...
    updateModifiedData(index: number) {
        if (this.thumbnailList[index]) {
            this._videoDataSetService
                .getThumbnailList(this.selectedProjectName, this.thumbnailList[index].uuid)
                .subscribe((res) => {
                    // @ts-ignore
    ...
    validateInputLabel = ({ target }: HTMLElementEvent<HTMLTextAreaElement>): void => {
        ...
        if (valTrimmed) {
            ...
            if (!isInvalidLabel) {
                ...
                console.log(this._selectMetadata.bnd_box);
