...
export class SegmentationCanvasService {
    ...
    mouseDownDraw(
        ...
        if (isNewPolygon && ctrlKey) {
            // console.log('if');
        } else if ((!this.returnTempPoly() && this.polygonIndex < 0 && this.clickPoint.pointIndex < 0) || altKey) {
    ...
    mouseMoveDraw(
        ...
        if (!isNewPolygon && isMouseDown) {
            ...
            this.setPolygonCoordinate(offsetX, offsetY, metadata, this.clickPoint);
            redrawCallback('draw');
        }
    ...
    pushTempPoint(mouseX: number, mouseY: number, imgX: number, imgY: number, len: number) {
        try {
            ...
            if (this.tmpPolygon) {
                ...
                this.generateNewTempPolygon(len);
                // !! original code by bobby, cannot remove below code or else no polygon coorPt metadata for canvas to draw
    ...
    mouseMovePolygon(
        ...
        try {
            ...
            const newXoffset = offsetX - this.globalXY.x;
            const newYoffset = offsetY - this.globalXY.y;
    ...
    setPolygonLineWidth(metadata: PolyMetadata, selectedPolygonIndex: number) {
        try {
            for (let i = 0; i < metadata.polygons.length; ++i) {
                if (i === selectedPolygonIndex) {
                    metadata.polygons[i].lineWidth = 2;
                } else {
                    metadata.polygons[i].lineWidth = 1;
                }
            }
        } catch (err) {
            ...
            if (this.tmpPolygon && this.tmpPolygon?.coorPt) {
                ...
                if (!closepath) {
                    ...
                    if (this.tmpPolygon) {
                        ...
                        this.setPolygonLineWidth(pol, pol.polygons.length - 1);
                        this.tmpPolygon = null;
                    }
                    this.setNewPolygon(false);
    ...
    returnTempPoly() {
        try {
            const result = this.tmpPolygon !== null && this.tmpPolygon !== undefined ? this.tmpPolygon : null;
            return result;
    ...
    setPolygonCoordinate(
        ...
        try {
            if (polygonIndex !== -1 && pointIndex !== -1) {
                metadata.polygons[polygonIndex].coorPt[pointIndex].x = mouseX;
                metadata.polygons[polygonIndex].coorPt[pointIndex].y = mouseY;
            }
        } catch (err) {
